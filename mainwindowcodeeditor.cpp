    ...
    , m_currentlyShownTooltipPange(-1, -1)
{
    ...
    // находим старый редактор из ui и заменяем
    int index = index = ui->splitter->indexOf(ui->codeEditor);;
    if (index != -1) {
        QWidget *oldEditor = ui->splitter->widget(index);
        ui->splitter->replaceWidget(index, m_codeEditor);
        delete oldEditor; // старый удаляем
    } else {
        qWarning() << "Не удалось найти CodeEditor в сплиттере для замены";
        setCentralWidget(m_codeEditor); // запасной план Б
    }
...
void MainWindowCodeEditor::onFileLoadingFinished(bool success, const QString &errorString)
{
    ...
    // данные в модели, редактор сам обновится
